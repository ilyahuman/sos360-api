...
// TODO: Replace with actual authenticated request type when auth is implemented
...
export class ContactController {
  ...
  assignContact = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.json({
        success: true,
        message: assignedUserId ? 'Contact assigned successfully' : 'Contact unassigned successfully',
        data: contact,
      });
    } catch (error) {
  ...
  updateFollowUp = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const contact = await this.contactService.updateFollowUp(
        id,
        followUpDate,
        companyId,
        userId
