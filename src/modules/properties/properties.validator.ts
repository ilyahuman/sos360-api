...
import { PropertyTypeEnum, Prisma } from '@prisma/client';
...
export function createPropertyValidationRules(): ValidationChain[] {
  return [
    ...
    body('surfaceTypes').optional().isArray().withMessage('Surface types must be an array'),
...
export function updatePropertyValidationRules(): ValidationChain[] {
  return [
    ...
    body('address').optional().isObject().withMessage('Address must be a valid object'),
...
export function listPropertiesValidationRules(): ValidationChain[] {
  return [
    ...
    query('divisionId').optional().isUUID().withMessage('Division ID must be a valid UUID'),
...
export function searchByLocationValidationRules(): ValidationChain[] {
  return [
    query('latitude')
      ...
      .withMessage('Latitude must be a floating-point number between -90 and 90')
      .customSanitizer(value => {
        // Optional: Convert to Prisma.Decimal with defined precision (e.g., 6)
    ...
    query('longitude')
      ...
      .withMessage('Longitude must be a floating-point number between -180 and 180')
      .customSanitizer(value => {
        // Optional: Convert to Prisma.Decimal with defined precision (e.g., 6)
        return new Prisma.Decimal(value).toDecimalPlaces(6);
      }),
...
export function getPropertyValidationRules(): ValidationChain[] {
  return [param('id').isUUID().withMessage('Property ID must be a valid UUID')];
}
...
export function getPropertiesByContactValidationRules(): ValidationChain[] {
  return [param('contactId').isUUID().withMessage('Contact ID must be a valid UUID')];
}
...
export function getPropertiesByDivisionValidationRules(): ValidationChain[] {
  return [param('divisionId').isUUID().withMessage('Division ID must be a valid UUID')];
}
