...
import { Property, Prisma, PrismaClient } from '@prisma/client';
import {
  CreatePropertyDTO,
  UpdatePropertyDTO,
  PropertyFiltersDTO,
...
export class PropertyRepository {
  ...
  async getStats(companyId: string, divisionId?: string): Promise<PropertyStatsDTO> {
    ...
    const propertiesByDivision = await this.prisma.property.groupBy({
      ...
      where: {
        ...where,
        divisionId: { not: null as any }, // Type assertion needed for Prisma groupBy with not-null filter
      },
      _count: {
        id: true,
      },
    ...
    propertiesByDivision.forEach(group => {
      if (group.divisionId && group._count) {
        byDivision[group.divisionId] = group._count.id || 0;
      }
    });
