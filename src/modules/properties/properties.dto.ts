...
import { Property, PropertyTypeEnum, Prisma } from '@prisma/client';
...
export interface CreatePropertyDTO {
  ...
  divisionId?: string; // Optional in DTO - will default to "General" division if not provided
...
export interface UpdatePropertyDTO {
  ...
  longitude?: number | null;
  primaryContactId?: string;
  divisionId?: string; // Must be valid division ID if provided
  totalArea?: number | null;
  surfaceTypes?: Prisma.InputJsonValue;
  parkingSpacesCount?: number | null;
  accessRestrictions?: string | null;
...
export interface PropertyResponseDTO {
  id: string;
  companyId: string;
  divisionId: string; // Always present - properties must belong to a division
  primaryContactId: string;
  name: string | null;
  propertyType: PropertyTypeEnum;
...
export class PropertyDTOMapper {
  static toResponse(
    ...
    const response: PropertyResponseDTO = {
      ...
      latitude: property.latitude ? property.latitude.toNumber() : null,
      longitude: property.longitude ? property.longitude.toNumber() : null,
  ...
  static toCreateInput(
    ...
    // divisionId must be provided by service layer (validated + defaulted to "General")
    if (!dto.divisionId) {
      throw new Error('divisionId is required - must be provided by service layer');
    }
  ...
  static toUpdateInput(
    ...
    if (dto.parkingSpacesCount !== undefined)
      updateData.parkingSpacesCount = dto.parkingSpacesCount;
    if (dto.accessRestrictions !== undefined)
      updateData.accessRestrictions = dto.accessRestrictions;
    if (dto.specialRequirements !== undefined)
      updateData.specialRequirements = dto.specialRequirements;
    if (dto.photos !== undefined) updateData.photos = dto.photos;
    if (dto.sitePlans !== undefined) updateData.sitePlans = dto.sitePlans;
    if (dto.spotonSiteProjectId !== undefined)
      updateData.spotonSiteProjectId = dto.spotonSiteProjectId;
    if (dto.autoSpotonSiteCreation !== undefined)
      updateData.autoSpotonSiteCreation = dto.autoSpotonSiteCreation;
    if (dto.mappingMethodPreference !== undefined)
      updateData.mappingMethodPreference = dto.mappingMethodPreference;
    if (dto.takeoffProgramPreference !== undefined)
      updateData.takeoffProgramPreference = dto.takeoffProgramPreference;
